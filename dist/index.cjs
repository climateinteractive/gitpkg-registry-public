...
function symbolForPredicateOp(op) {
  switch (op) {
    ...
    case "approx":
      return "\u2248";
    default:
      (0, import_assert_never.assertNever)(op);
  }
}
  ...
  if (checkScenario.spec === void 0) {
    if (checkScenario.error) {
      switch (checkScenario.error.kind) {
        ...
        default:
          (0, import_assert_never2.assertNever)(checkScenario.error.kind);
      }
    } else {
  ...
  function positionName(position) {
    switch (position) {
      ...
      case "at-maximum":
        return "maximum";
      default:
        (0, import_assert_never2.assertNever)(position);
    }
  }
  ...
  if (result.status === "error") {
    if (result.message) {
      ...
      switch (result.errorInfo.kind) {
        ...
        default:
          (0, import_assert_never2.assertNever)(result.errorInfo.kind);
      }
    } else {
...
function inputValueAtPosition(inputVar, position) {
  switch (position) {
    ...
    default:
      (0, import_assert_never4.assertNever)(position);
  }
}
      ...
      if (typeof predOp.dataset === "string") {
        switch (predOp.dataset) {
          case "inherit":
            ...
            break;
          default:
            (0, import_assert_never5.assertNever)(predOp.dataset);
        }
      } else {
      ...
      if (typeof predOp.scenario === "string") {
        switch (predOp.scenario) {
          case "inherit":
            ...
            break;
          default:
            (0, import_assert_never5.assertNever)(predOp.scenario);
        }
      } else {
...
function parseComparisonSpecs(specSource) {
  ...
  switch (specSource.kind) {
    ...
    case "yaml":
      ...
      break;
    default:
      (0, import_assert_never9.assertNever)(specSource.kind);
  }
  if (validate(parsed)) {
